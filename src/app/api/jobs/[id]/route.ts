...
export async function GET(
  ...
  try {
    ...
    if (!job) {
      return NextResponse.json({ message: "ไม่พบข้อมูลงาน" }, { status: 404 });
    }
    ...
    const updatedJob = await (Job as any).findByIdAndUpdate(
      id,
      {
        $set: {
          ...
          status: allowedStatus.includes(data.status) ? data.status : existing.status,
        },
      },
      { new: true }
    ).lean().exec();
