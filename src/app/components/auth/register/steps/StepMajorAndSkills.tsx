...
import { MAJOR_OPTIONS } from "@/app/constants/Majors";
...
function StepMajorAndSkills({
  ...
  const getDescription = () => {
    ...
    if (data.role === "alumni")
      return "ระบุวิชาเอก อัปโหลดรูปภาพ และอีเมลอาจารย์์ที่ติดต่อได้";
        ...
        <div className="w-full mb-2">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer group">
              ...
              />
              <span
                className={`text-sm ${teacherType === "academic" ? "text-gray-900 font-medium" : "text-gray-600"}`}
              >
                อาจารย์
              </span>
            </label>
            ...
            <label className="flex items-center gap-2 cursor-pointer group">
              ...
              />
              <span
                className={`text-sm ${teacherType === "staff" ? "text-gray-900 font-medium" : "text-gray-600"}`}
              >
      ...
      {data.role === "student" && (
        <div className="mt-2 w-full">
          ...
          <p className="text-[10px] text-gray-400 text-center mt-2">
            * แนะนำให้ระบุอย่างน้อย 5 ทักษะ เพื่อความแม่นยำ
          </p>
        ...
        <>
            ...
            <div className="flex flex-col gap-3">
              {(data.teacherEmails || ["", ""]).map((email, index) => (
                <div key={index} className="relative">
                  ...
                  {/* ปุ่มลบอีเมล */}
                  {(data.teacherEmails?.length || 0) > 2 &&
                    index === (data.teacherEmails?.length || 0) - 1 && (
                      <button
                        type="button"
          ...
          <div className="w-full">
            ...
            <p className="text-xs text-gray-400 mt-1">
              * โปรดใช้รูปที่เห็นใบหน้าชัดเจน ไม่ใส่แว่นทึบ หมวก
              หรือวัตถุที่ปิดบังใบหน้า
            </p>
            <p className="text-xs text-gray-400 mt-1">
              * ระบบจะส่งข้อมูลของคุณไปยังอีเมลเหล่านี้เพื่อยืนยันตัวตน
            </p>
          </div>
        </>
      )}
